...
import { BasicPhysicsEvent } from 'src/events/basicPhysicsEvent';
import { EventType } from 'src/events/event';
...
import { Collide } from 'src/physics/basic/interactionTypes';
import { QuadTree } from 'src/physics/basic/quadTree';
import { RayHitList } from 'src/physics/basic/rayHit';
...
export class SBasicPhysics extends System {
  ...
  private interactionEvents: BasicPhysicsEvent[] = [];
      ...
      if (body.active) {
        for (const body2 of body.wasCollidingWith) {
          if (!body.collidingWith.includes(body2)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.COLLISION_END, body, body2));
          }
        }

        for (const body2 of body.wasTriggeredBy) {
          if (!body.triggeredBy.includes(body2)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.TRIGGER_END, body, body2));
          }
        }
      }
    }
    ...
    if (body1.mask.has(body2.group.value) && body2.mask.has(body1.group.value) && this.intersects(body1, body2)) {
      if (body1.bodyType === 'dynamic' && !body1.isTrigger && !body2.isTrigger) {
        ...
        if (!body1.wasCollidingWith.includes(body2)) {
          if (!this.hasInteraction(BasicPhysicsEvent.COLLISION_START, body1, body2)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.COLLISION_START, body1, body2));
          }
        } else {
          if (!this.hasInteraction(BasicPhysicsEvent.COLLISION_STAY, body1, body2)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.COLLISION_STAY, body1, body2));
          }
        }
        ...
        if (!body1.wasTriggeredBy.includes(body2)) {
          if (!this.hasInteraction(BasicPhysicsEvent.TRIGGER_START, body1, body2)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.TRIGGER_START, body1, body2));
          }
        } else {
          if (!this.hasInteraction(BasicPhysicsEvent.TRIGGER_STAY, body1, body2)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.TRIGGER_STAY, body1, body2));
          }
        }
        ...
        if (!body2.wasTriggeredBy.includes(body1)) {
          if (!this.hasInteraction(BasicPhysicsEvent.TRIGGER_START, body2, body1)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.TRIGGER_START, body2, body1));
          }
        } else {
          if (!this.hasInteraction(BasicPhysicsEvent.TRIGGER_STAY, body2, body1)) {
            this.interactionEvents.push(BasicPhysicsEvent.get(BasicPhysicsEvent.TRIGGER_STAY, body2, body1));
          }
        }
  ...
  private hasInteraction(type: EventType<BasicPhysicsEvent>, body1: CBasicBody, body2: CBasicBody): boolean {
